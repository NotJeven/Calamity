...
# 2 - Shattering by Moesh
    ...
    # 4 - Adversity by Jeven
    # Load a new arena
    execute if score #arenaAction gameVariable matches 0 if score #currentArena gameVariable matches 4 run function #calamity:load_arena_2_shattering
    # Erase the arenas
    execute if score #arenaAction gameVariable matches 1 if score #currentArena gameVariable matches 4 run function calamity:arena/shattering/erase
    # Additional actions
    execute if score #arenaAction gameVariable matches 2 if score #currentArena gameVariable matches 4 run function calamity:arena/shattering/actions
    # Count the ore
    execute if score #arenaAction gameVariable matches 3 if score #currentArena gameVariable matches 4 run execute store result score OreLeft gameVariable run fill 133 21 176 139 0 182 minecraft:petrified_oak_slab[type=double] replace minecraft:iron_ore
    execute if score #arenaAction gameVariable matches 3 if score #currentArena gameVariable matches 4 run fill 133 21 176 139 0 182 minecraft:iron_ore replace minecraft:petrified_oak_slab
